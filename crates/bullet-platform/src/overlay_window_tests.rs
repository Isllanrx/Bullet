...
fn test_hidden_client_hides_the_overlay() {
    assert!(decide_placement(ClientWindowState::Hidden, true).is_none());
    assert!(decide_placement(ClientWindowState::Absent, true).is_none());
}

#[test]
fn test_overlay_is_not_shown_when_not_wanted() {
    assert!(
        decide_placement(ClientWindowState::Visible(client_rect()), false).is_none(),
...
fn test_visible_client_places_the_overlay_alongside_it() {
    let placement = decide_placement(ClientWindowState::Visible(client_rect()), true)
